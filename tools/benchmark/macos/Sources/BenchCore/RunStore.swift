...
public struct RunStore: Sendable {
  ...
  private func offlineHTML(_ report: LoadedReport) -> String {
    ...
    return """
      ...
      <main><small>USHAS / RENDER LAB · OFFLINE REPORT</small><h1>Every frame<br>has a character.</h1><strong>\(escape(score))</strong><p>\(escape(report.problem ?? "Completed-render rate. This does not measure GPU busy time or physical panel delivery."))</p><p>Profile: \(escape(report.report.profileVersion))</p><table>\(arms)</table><h2>Retained images</h2>\(images)<p><a href="result.json">Portable result</a> · <a href="originals/result.json">Original structured result</a> · Source \(escape(report.report.sourceRevision ?? "unavailable"))</p></main></html>
      """
  }
}
